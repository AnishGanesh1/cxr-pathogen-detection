"""
Train p19 → p16 → p17 incrementally (each picks up from the previous best checkpoint),
then produce a visual inference PNG for each group (same style as evaluate.py).

Training order:
  1. p19  — starts from checkpoints_mimic/best_mimic_model.pth  (from train_mimic.py)
  2. p16  — starts from checkpoints_mimic/best_p19_model.pth
  3. p17  — starts from checkpoints_mimic/best_p16_model.pth
"""
...
# Training order: p19 first (from existing base), then p16, then p17
GROUPS = [
    # (name,  patient_prefix,  resume_checkpoint,                       save_best_as)
    ("p19", "19", f"{CKPT_DIR}/best_mimic_model.pth",  f"{CKPT_DIR}/best_p19_model.pth"),
    ("p16", "16", f"{CKPT_DIR}/best_p19_model.pth",    f"{CKPT_DIR}/best_p16_model.pth"),
...
def train_group(name, patient_prefix, resume_path, save_best_as):
    ...
    if resume_path and os.path.exists(resume_path):
        ...
        print(f"[INFO] No checkpoint found at {resume_path}. Training from scratch.\n")
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"\n[{name}] Epoch {epoch+1} — Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}")
...
if __name__ == "__main__":
    # ── Step 1: Train p19 → p16 → p17 sequentially ──────────────────────────
    for (name, prefix, resume, save_as) in GROUPS:
        train_group(name, prefix, resume, save_as)

    # ── Step 2: Visual inference for all three groups ────────────────────────
    print("\n" + "="*60)
    print("  POST-TRAINING VISUAL INFERENCE (p19 + p16 + p17)")
    print("="*60)

    inference_map = {
        "p19": (f"{CKPT_DIR}/best_p19_model.pth", "inference_output_p19.png"),
    ...
    for grp, (ckpt, out_png) in inference_map.items():
        # Find one sample image from the corresponding patient folder
        imgs = glob.glob(
        ...
        if os.path.exists(ckpt):
            ...
            print(f"[SKIP] {grp}: checkpoint not found at {ckpt}")

    print("\n🎉 All done!")
    print("  ✔ inference_output_p19.png")
    print("  ✔ inference_output_p16.png")
    print("  ✔ inference_output_p17.png")
