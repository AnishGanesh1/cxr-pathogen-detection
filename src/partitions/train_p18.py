...
RESUME_FROM = "checkpoints_mimic/best_p17_model.pth"
SAVE_BEST   = "checkpoints_mimic/best_p18_model.pth"
CKPT_DIR    = "checkpoints_mimic"

PATIENT_PREFIX = "18"   # filters only p18 patients
BATCH_SIZE     = 32
EPOCHS         = 10
LR             = 0.001
NUM_CLASSES    = 14
...
if os.path.exists(RESUME_FROM):
    ...
    print("✅ Loaded p17 knowledge successfully!\n")
else:
...
for epoch in range(EPOCHS):
    ...
    torch.save({
        ...
        "val_loss": val_loss,
    }, f"{CKPT_DIR}/p18_epoch_{epoch}.pth")
...
print(f"\n✅ p18 training complete. Best val loss: {best_val_loss:.4f}")
print(f"   Best model saved to: {SAVE_BEST}")

# ─── Post-Training Visual Inference ──────────────────────────────────────────
print("\n" + "="*60)
print("  RUNNING VISUAL INFERENCE ON p18 SAMPLE")
print("="*60)
...
# Find a sample image for p18
imgs = glob.glob(f"{ROOT_DIR}/files/p18/**/*.jpg", recursive=True)
if not imgs:
    print("[WARN] No p18 sample images found for inference.")
    sys.exit(0)
sample_img_path = imgs[0]
output_png = "inference_output_p18.png"
...
fig.suptitle(f"MIMIC-CXR Pathology Detection — p18", fontsize=14, fontweight='bold')
...
axes[1].axis('off')
y = 0.95
axes[1].text(0.05, y, "Pathology Detections (p18):", fontsize=15, fontweight='bold', transform=axes[1].transAxes)
y -= 0.1
detected_any = False
